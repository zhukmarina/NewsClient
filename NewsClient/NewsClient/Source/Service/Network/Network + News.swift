...
protocol NetworkServiceNews {
    ...
    func searchNews(for searchIn: String, completion: @escaping NewsInfoCompletion)
...
extension NetworkService: NetworkServiceNews {
    ...
    func searchNews(for searchIn: String, completion: @escaping NewsInfoCompletion) {
        ...
        guard !searchIn.trimmingCharacters(in: .whitespaces).isEmpty else{
            return
        }
            urlString = "\(APIConstants.searchNewsUrl())?q=\(searchIn)&apiKey=\(APIConstants.appId)"
